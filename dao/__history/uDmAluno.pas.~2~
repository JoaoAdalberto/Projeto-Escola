...
type
  TdmAluno = class(TDataModule)
    ...
    sqlSelectAlunoALUSER: TIntegerField;
...
begin
  ...
  sqlAlterarAluno.Params[11].AsInteger  := oAluno.ALUSER;
...
begin
  ...
  try
    ...
    oAluno.ALUSER := sqlAluno.FieldByName('ALUSER').AsInteger;
...
begin
  ...
  try
    ...
    sqlSequencia.CommandText := 'select coalesce(max(ESCCOD), 0) + 1 as seq from Aluno';
    sqlSequencia.Open;
...
begin
  ...
  sqlInserirAluno.Params.FindParam('ALUESC').AsInteger := GerarCod;
  ...
  sqlInserirAluno.Params.FindParam('ALUSER').AsInteger := oAluno.ALUSER;
